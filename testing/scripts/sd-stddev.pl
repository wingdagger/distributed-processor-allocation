...
#    push (@its, "3");
	...
	foreach $tst (@tsts)
	{
		...
		foreach $it (@its)
		{
		    $max_unfair_avg += `gawk -f avg.awk $HOME/thesis/testing/$alg/$env/$tst/iter$it/stddev-data.ds`;
		    $max_unfair_stddev += `$HOME/thesis/testing/scripts/stddev.pl $HOME/thesis/testing/$alg/$env/$tst/iter$it/stddev-data.ds $max_unfair_avg`
		}
		$cnt = @its;
		...
		print "$alg, $env, $tst : \n";
		print "\tAvg. Std Dev: $max_unfair_avg_its\n";
		print "\tStdDev of STddev: $mu_stddev\n";
		print "--------------\n";
	    }
	}
    }
